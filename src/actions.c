   ...
   qs_action_t *action)
{
   ...
   /* if there's a resource, run it. */
      ...
      e->rlink = qs_rlink_push_at (e->rlink->object, r,
         e->rlink->priority, e->rlink, e->rlink->child_back, e,
         action);
      rval = qs_execute_rlink (e, e->rlink);
   }
   ...
   qs_action_t *action)
{
   ...
   if ((length = qs_value_length (val)) < 0) {
      p_error (action->value->node, "cannot index value of type '%s'.\n",
         qs_value_type (val));
